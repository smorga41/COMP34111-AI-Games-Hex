...
@njit
def is_bridging_move(simulation_board, board_size, move_idx, current_player, parent, rank, neighbour_displacements):
    """
    Determines if placing at move_idx connects two or more separate components.
    """
    # Set the move temporarily
    simulation_board[move_idx] = current_player

    # Keep track of unique roots of neighboring same-color stones
    unique_roots = 0
    neighbor_roots = np.empty(0, dtype=np.int32)
    ...
    for d in range(6):
        ...
        if 0 <= nx < board_size and 0 <= ny < board_size:
            ...
            if simulation_board[n_idx] == current_player:
                root = find(parent, n_idx)
                # Check if this root is already counted
                already_counted = False
                for r in range(neighbor_roots.size):
                    if neighbor_roots[r] == root:
                        already_counted = True
                        break
                if not already_counted:
                    if unique_roots < board_size * board_size:  # Prevent overflow
                        neighbor_roots = np.append(neighbor_roots, root)
                        unique_roots += 1
                if unique_roots >= 2:
                    # Reset the move before returning
                    simulation_board[move_idx] = EMPTY
                    return True
    # Reset the move before returning
    simulation_board[move_idx] = EMPTY
    return False
...
@njit
def simulate_random_playoff_numba(simulation_board, board_size, player, neighbour_displacements):
    ...
    while True:
        ...
        num_available = available_moves.size
        if num_available == 0:
            return 1 - current_player  # Opposite player as fallback

        # Identify bridging moves
        bridging_moves = np.empty(0, dtype=np.int32)
        for i in range(num_available):
            move_idx = available_moves[i]
            if is_bridging_move(simulation_board, board_size, move_idx, current_player, parent, rank, neighbour_displacements):
                bridging_moves = np.append(bridging_moves, move_idx)

        if bridging_moves.size > 0:
            # Prefer bridging moves
            selected_move = bridging_moves[np.random.randint(bridging_moves.size)]
        else:
            # Select randomly as usual
            selected_move = available_moves[np.random.randint(num_available)]

        # Apply the selected move
        simulation_board[selected_move] = current_player
        x = selected_move // board_size
        y = selected_move % board_size

        # Union with virtual edges if needed
        if current_player == RED:
            if x == 0:
                union(parent, rank, selected_move, red_top)
            if x == board_size - 1:
                union(parent, rank, selected_move, red_bottom)
        elif current_player == BLUE:
            if y == 0:
                union(parent, rank, selected_move, blue_left)
            if y == board_size - 1:
                union(parent, rank, selected_move, blue_right)
        ...
        for d in range(6):
            ...
            if 0 <= nx < board_size and 0 <= ny < board_size:
                n_idx = nx * board_size + ny
                if simulation_board[n_idx] == current_player:
                    union(parent, rank, selected_move, n_idx)
...
class TreeNode:
    ...
    def simulate_random_playoff(self):
        ...
        result_int = simulate_random_playoff_numba(simulation_board, self.board_size, self.player, NEIGHBOUR_DISPLACEMENTS)
        return result_int
...
class MCTS:
    def __init__(self, iterations: int = 20000, c_param: float = 0.5, time_limit: float = 5.0):
        ...
        simulate_random_playoff_numba(dummy_board.copy(), 1, RED, NEIGHBOUR_DISPLACEMENTS)
